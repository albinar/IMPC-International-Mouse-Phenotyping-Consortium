...
preProcessingFunc <- function(inputPath, inputCSV, centre, barcodeExpression){
  ...
  mouseID.colNames <- c("Mouse_ID", "Experiment_Number", "Animal.ID")
  ...
  controlNames <- c("CTRL", "FMO", "UNSTAINEDUNSTAINED", "UNSTAINED", "Beads", "Neg", "DAPI", "SingleStaincells",
                    "_Nikolai", "WT", "WT1", "WT2", "WT3", "WT4", "KO1", "KO2", "KO3", "KO4", "Compensation Controls" ) ## Adding the control names since most of centres except for Sanger have control files together with main FCS files
  ...
  for(i in 1:numPaths){
    ...
    rownames(store.allFCS.temp) <- 1:nrow(store.allFCS.temp)
    if(centre == "sanger" | centre == "tcp" | centre == "jax" | centre == "gmc"|centre == "ucd"){
