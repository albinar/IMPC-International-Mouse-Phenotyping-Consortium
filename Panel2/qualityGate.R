...
qualityGate <- function(f, scat.chans, channels.ind, centre, panel.no){
  ...
  is.DAPI <- ((length(grep('DAPI', pData(parameters(f))$desc)) > 0))
  ...
  if((singlets.peak - gate) > 20000){ ## Changing this to 20000 from 50000
  ...
  if(length(grep(colnames(f),pattern = "SSC|SS-*")) < 3){  
    temp <- getflowFrame(temp2)@exprs[, c(scat.chans['SSC-W'], scat.chans['FSC-H'])]
  }else{
